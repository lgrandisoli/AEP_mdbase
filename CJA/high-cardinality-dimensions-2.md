...
created_at: "2026-06-23T20:43:29.511310+00:00"
---
...
When you request a report containing a dimension with too many unique values, Analysis Workspace shows an indicator in the dimension header stating that not all dimension items are included. For example, **Rows: 1-50 of more than 22,343,156**. The **more than** keyword indicates that some optimization was applied to the report to return the most important dimension items.
...
Though some individual dimension items might be missing from high cardinality reports, column totals are accurate and not based on truncated data. The [Approximate Count Distinct](/en/docs/analytics-platform/using/cja-components/cja-calcmetrics/cm-adv-functions#approximate-count-distinct) calculated metric function is also not affected by truncated dimension items.
...
- Use a [segment](/en/docs/analytics-platform/using/cja-components/segments/seg-create). Segments apply at the time that each server processes a subset of data.
...
- Use the [Include/exclude](/en/docs/analytics-platform/using/cja-dataviews/component-settings/include-exclude-values) component setting in the data view manager.
...
- Consider using [Full Table Export](/en/docs/analytics-platform/using/cja-workspace/export/export-cloud) to return all rows of the table.
- If the number of unique values is the primary focus, use the [Approximate Count Distinct](/en/docs/analytics-platform/using/cja-components/cja-calcmetrics/cm-adv-functions#approximate-count-distinct) calculated metric function.
