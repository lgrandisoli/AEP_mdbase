...
created_at: "2026-06-23T20:41:56.748064+00:00"
---
...
, and so on.
Lookup
You can add datasets as lookups of fields within all dataset types: Profile, Lookup, and Event datasets (the latter was always supported). This additional capability expands the capability of Customer Journey Analytics to support complex data models, including B2B. This data is used to look up values or keys found in your Event, Profile, or Lookup data. You can add up to three levels of lookups. For example, you might upload lookup data that maps numeric IDs in your event data to product names. See the [B2B example](/en/docs/analytics-platform/using/cja-usecases/b2b/example) for an example.**Note**:

- [Derived Fields](/en/docs/analytics-platform/using/cja-dataviews/derived-fields) cannot be used as matching keys for lookups within Connections.
- Large lookups are not supported for [Customer Managed Keys](/en/docs/analytics-platform/using/cja-privacy/cmk) and [Ad hoc datasets](#ad-hoc-dataset).
