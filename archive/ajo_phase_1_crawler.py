...
ROOT_PREFIX = "/pt-br/docs/journey-optimizer"
...
def save_page(page: Page, output_dir: Path) -> Path:
    ...
    topic_dir = category_dir / page.topic_path
    topic_dir.mkdir(parents=True, exist_ok=True)

    base = friendly_filename(page.title, page.section_slug)
    target = topic_dir / f"{base}.md"

    n = 2
    while target.exists():
        target = topic_dir / f"{base}-{n}.md"
        n += 1
...
def build_readme(output_dir: Path, manifest: List[dict]) -> None:
    ...
    for category in ["guides", "tutorials", "release-notes", "resources", "other"]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--start-url",
        default="https://experienceleague.adobe.com/pt-br/docs/journey-optimizer",
        help="URL inicial",
    )
    parser.add_argument(
        "--output-dir",
        default=r"/Users/luizcarlosgrandisoli/Library/CloudStorage/OneDrive-Adobe/RFP_Knowledge/AJOB2C",
    ...
    parser.add_argument(
        ...
        help="Categorias a manter no recorte inicial. Ex: guides tutorials",
    )
    parser.add_argument(
        "--include-all",
        action="store_true",
        help="Inclui guides, tutorials, release-notes, resources e other",
    )
