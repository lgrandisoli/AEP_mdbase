...
def extract_links_with_context(soup: BeautifulSoup, base_url: str) -> List[Tuple[str, str]]:
    """
    Extract every internal documentation link we can find in the rendered page.

    We look inside the main article first, then fall back to the whole body so we do
    not miss links that are rendered in cards, callouts, related-content blocks, or
    secondary navigation inside the page.
    """
