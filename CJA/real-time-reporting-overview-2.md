...
created_at: "2026-06-23T20:41:54.603873+00:00"
---
...
You must have the
Ultimate
package in order to use the functionality described in this section. Contact your administrator if you’re unsure which Customer Journey Analytics package you have.
TIP
If you are entitled to the Ultimate package but do not see the
Real-time refresh toggle
, create a customer care ticket to request to enable real-time reporting for your organization.
## Use cases
...
- The most valuable use cases for real-time reporting are around major sales, promotions, or product launches. As part of that launch you want to know: How do the sales compare to your last sale? How does this product launch compare to the last product launch? Do your promotions for this important day or event actually work?
- Relevant, but less valuable uses cases for real-time reporting are validation use cases. You want to validate, for example: Is the campaign journey you recently launched actually working? When your new product page went live, are you collecting customer data from the page? Is your live media event going ok?

Do not consider real-time reporting for operations monitoring use cases. For example, to answer the question whether a site is properly working. Since the [real-time refresh toggle](/en/docs/analytics-platform/using/cja-components/real-time-reporting/use-real-time) automatically disables after 30 minutes and the real-time report stops refreshing, you should not use a real-time report as a reliable source for these use cases.

## How it works

Real-time reporting uses a consolidated dataset that is completely separate from the [consolidated (combined) dataset](/en/docs/analytics-platform/using/cja-connections/combined-dataset) used for standard reporting. You use the [Real-time refresh toggle](/en/docs/analytics-platform/using/cja-components/real-time-reporting/use-real-time) to switch between:
...
- When you use the download or export options to download a project or export data from a freeform table, consider the following: A downloaded CSV project or exported CSV file contains the real-time data available at the moment of download or export. A downloaded PDF project contains non real-time data, similar to the data that is shown when real-time refresh is disabled.
