#!/usr/bin/env python3
"""
Phase 1 — Adobe Experience Cloud AI Agent Orchestrator Knowledge Builder

What it does:
- Crawls Adobe Experience Cloud AI documentation starting from one or more seed URLs
- Keeps only internal documentation links under /en/docs/experience-cloud-ai/experience-cloud-ai/agents/agent-orchestrator
...
ROOT_PREFIX = "/en/docs/experience-cloud-ai/experience-cloud-ai/agents/agent-orchestrator"
...
def infer_topic_path(url: str, breadcrumbs: List[str], title: str) -> str:
    ...
    tail = [slugify(p) for p in tail if p and p not in {"en", "docs", "experience-cloud-ai", "agents", "agent-orchestrator"}]
    if tail:
        return "/".join(tail[:3])
...
def build_readme(output_dir: Path, manifest: List[dict]) -> None:
    ...
    lines: List[str] = []
    lines.append("# Experience Cloud AI Agents Knowledge Index")
    lines.append("")
    lines.append("Generated from Adobe Experience Cloud AI Agents documentation.")
    lines.append("")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Crawler do Adobe Experience Cloud AI para Markdown em pasta plana"
    )
    parser.add_argument(
        "--start-urls",
        nargs="*",
