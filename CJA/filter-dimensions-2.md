...
created_at: "2026-06-23T20:45:38.688002+00:00"
---
...
- Select a cell in the data block.
- Select Edit data block in the Commands panel.
- Select Next to display the Dimensions tab.
- Select next to a component name in the table. {modal="regular"}
- Select Filter dimension in the pop-up menu to display the Filter dimension pane.
- Select Most popular or Specific as the Type . {modal="regular"}
...
- Select for the preferred metric. {modal="regular"}
...
The **Most popular** option allows you to dynamically filter dimension items based on metric values. Most popular returns the highest ranked dimension items based on metric values. By default, the first 10 dimensions items are listed, sorted by the first metric added to the data block.

{modal="regular"}

#### Page and Rows options

Use the **Page** and **Rows** fields to divide data into sequential groups or pages. This feature allows you to pull ranked row values other than the top-most values into your report. And is especially useful for pulling data beyond the 50,000 row limit.
...
In Customer Journey Analytics, some dimensions collect a *No value* entry. The **Include “No value”** setting allows you to exclude these values from reports. For example, you can create a classification such as the Product Name classification based on the Product SKU key. If a specific product SKU has not been set up with its specific Product Name classification, its Product Name value is set to *No value*.
...
- Select an operator from the operator drop-down menu. By default Contains the phrase is selected {modal="regular"}
- Enter a search term.
- Select Add row to confirm the selection and add another criteria item.
- Select to remove a criteria item.
...
- Select the From list option to search for and select dimension items. When you select the From list option, the Dimension items list is populated with dimension items ordered by number of events. {modal="regular"}
- Enter a search term in the Add item to search the list.
- To search for an item not included in the last 90 days of data, select Show items for the last 6 months to extend the search. After data from the past 6 months loads, Report Builder updates the link to Show items for last 18 months .
- To delete an item from the Selected items list, select .
- To move an item in the Selected items list, drag and drop the item or select to display the context menu and select from the move options.
- Select Apply .
...
- Select for a dimension. For example, Interaction channel .
- Double select a dimension items to add to the filter. Alternatively, select one or more dimension items and drag and drop the selection onto the Row section. {modal="regular"}
