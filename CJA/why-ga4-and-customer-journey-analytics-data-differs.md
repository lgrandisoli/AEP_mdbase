...
created_at: "2026-06-23T20:46:03.375894+00:00"
---
...
- In Analysis Workspace, select the **+** icon near the metrics list to open the Calculated Metric Builder.
- Name it “Engagement Rate” and set the format to **Percent**.
- Define the formula as your engaged sessions segment divided by **Sessions**.
- Select **Save**.
...
Customer Journey Analytics also provides a built-in **Bounce Rate** metric, but its default definition differs: it counts sessions in which only a single event was recorded, which is directionally opposite to GA4’s definition for many sites. Comparing GA4’s bounce rate to the default Bounce Rate metric — rather than to your 1 - Engagement Rate calculation — produces materially different numbers.
...
Inactivity timeout
Adjustable property-wide (30-minute default, up to 7 hours 55 minutes)
Configurable per data view
Session-starting events
session_start
only (automatic)
Configurable; any event can start a session
Session-ending events
None
Configurable; any event can end a session
...
GA4’s primary user metric, **Active users**, counts users who had at least one engaged session in the date range. The **People** metric in Customer Journey Analytics counts unique person IDs in the date range.

Expect these metrics to differ for several reasons:

- **Engagement threshold**: GA4 Active users excludes visitors who had no [engaged session](#engaged-sessions). The People metric in Customer Journey Analytics includes everyone regardless of engagement level.
- **Stitching**: If stitching is enabled, a person who visited from both a mobile device and a desktop can be counted as one person in Customer Journey Analytics but two users in GA4. Stitching typically makes the People metric lower than GA4 users on stitched datasets.
...
- User-ID (if set by your implementation)
- Google Signals (if the user is signed into a Google account with personalization enabled)
- Device ID (cookie-based client ID)

In most implementations, that person ID is an ECID (Experience Cloud ID). The optional **Stitching** feature can then resolve cross-device and cross-channel identities using field-based or graph-based methods, allowing a mobile app session and a desktop browser session to be associated with the same person.
...
- **Data view settings**: An [attribution model](/en/docs/analytics-platform/using/cja-dataviews/component-settings/attribution) can be set on any metric component in the data view, establishing the default for that metric across all reports. No attribution model is applied by default. You can configure a data view to contain multiple copies of the same metric, each using a different default attribution model.
- **Component-level override**: After dragging a metric into a Freeform table, right-click its column header and select **Use non-default attribution model** to override it for that instance. You can also drag the same metric into the table multiple times, each using a different attribution model for a direct side-by-side comparison.
...
- **Ingestion review**: If you suspect the discrepancy originates in how GA data was brought into Platform rather than in reporting definitions, review the ingestion setup in [Migrate data from Google Analytics](/en/docs/analytics-platform/using/cja-usecases/third-party/ga/overview).
