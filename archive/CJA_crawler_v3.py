#!/usr/bin/env python3
"""
Phase 1 — Adobe Customer Journey Analytics Knowledge Builder

What it does:
- Crawls Adobe Customer Journey Analytics documentation starting from one or more seed URLs
- Keeps only internal documentation links under /en/docs/analytics-platform/using/cja-overview and follows all internal anchors it finds in the page content
...
ROOT_PREFIX = "/en/docs/analytics-platform/using/cja-overview"
ALLOWED_PREFIXES = (
    ROOT_PREFIX,
)
...
def is_doc_url(url: str) -> bool:
    parsed = urlparse(url)
    if parsed.path.endswith((".pdf", ".zip", ".png", ".jpg", ".jpeg", ".svg", ".gif", ".webp")):
        return False
...
def infer_topic_path(url: str, breadcrumbs: List[str], title: str) -> str:
    ...
    root_segments = [seg for seg in ROOT_PREFIX.split("/") if seg]
    tail = parts
    for i in range(len(parts) - len(root_segments) + 1):
        if parts[i : i + len(root_segments)] == root_segments:
            tail = parts[i + len(root_segments) :]
            break

    tail = [slugify(p) for p in tail if p and p not in {"en", "docs", "experience-platform", "analytics-platform"}]
    if tail:
        return "/".join(tail[:3])
...
URL_TOKEN_RE = re.compile(r"""(?:https?://experienceleague\.adobe\.com/[^"\'\s<>]+|/en/docs/[^"\'\s<>]+)""")
...
def _extract_urls_from_text(text: str) -> List[str]:
    ...
    candidates = []
    for match in URL_TOKEN_RE.finditer(cleaned):
...
def extract_links_with_context(soup: BeautifulSoup, base_url: str, raw_html: str = "") -> List[Tuple[str, str]]:
    """Extract every internal documentation link we can find.

    This scans:
    1) normal anchor tags in the main content and body,
    2) other link-like attributes on any tag, and
    3) raw HTML text for URLs embedded in scripts or data blobs.

    The goal is to avoid missing pages when the site exposes links outside standard
    anchors or when parts of the navigation are rendered server-side in JSON.
    """
    ...
    for sel in ["main", "article", '[role="main"]', "body"]:
        node = soup.select_one(sel)
        if node and node not in search_roots:
            search_roots.append(node)

    if not search_roots:
        search_roots = [soup]

    def add_url(candidate: str, section: str = "") -> None:
        url = normalize_url(base_url, candidate)
        if url and is_doc_url(url) and url not in seen:
            result.append((url, section))
            seen.add(url)
    ...
    for root in search_roots:
        ...
        for node in root.find_all(True):
            ...
            # Standard anchors.
            if node.name == "a":
                add_url(node.get("href") or "", current_section)

            # Other commonly used link-like attributes in Adobe pages.
            for attr in ("href", "data-href", "data-url", "data-path", "content"):
                val = node.get(attr)
                if isinstance(val, str) and val:
                    for candidate in _extract_urls_from_text(val):
                        add_url(candidate, current_section)

    # Final fallback: scan the raw HTML for any embedded URLs under the target tree.
    for candidate in _extract_urls_from_text(raw_html):
        add_url(candidate, "")
...
def build_readme(output_dir: Path, manifest: List[dict]) -> None:
    ...
    lines.append("")
    lines.append("Generated from Adobe Customer Journey Analytics documentation.")
    lines.append("")
...
def crawl(
    ...
    while queue and saved_count < max_pages:
        ...
        soup = BeautifulSoup(html, "html.parser")
        for link_url, section_ctx in extract_links_with_context(soup, url, html):
            if link_url in visited:
                continue
            # Keep the section context when it is useful, but never use it to block crawling.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Crawler do Adobe Customer Journey Analytics para Markdown em pasta plana"
    )
    parser.add_argument(
        "--start-urls",
        nargs="*",
        default=DEFAULT_START_URLS,
        help="URLs iniciais (uma página raiz já basta, mas você pode passar mais seeds)",
    )
    parser.add_argument(
        "--output-dir",
        default="./CJA",
    ...
    only_categories = None if args.include_all or args.only is None else args.only
