...
created_at: "2026-06-23T20:45:09.697055+00:00"
---
...
In the [Calculated metrics manager](/en/docs/analytics-platform/using/cja-components/cja-calcmetrics/cm-workflow/cm-manager), you can share calculated metrics. Depending on your permissions, you can share calculated metrics with your whole organization, groups, or individual users:
...
- In the Calculated metric manager , select one or more of the calculated metrics you want to share.
- From the action bar, select Share .
- In the Share Calculated metric dialog: (optionally) use to Search individuals or groups for and limit the list of groups or individuals you want to share the calculated metrics with. Select one or more options from the Organization or Groups section or search for and select one or more individuals. What options are available depends on your role. Select Save to share the calculated metrics. Select Cancel to cancel.

## Best practices

Below are some best practices when you should share calculated metrics and with whom you should share calculated metric.

- As an administrator, only share a calculated metric with All if you are convinced anyone in your organization is comfortable using the calculated metric. You can also consider favoring these calculated metrics. See Mark a calculated metric as favorite for more information.
